...
def getParam(arr, pos, mode):
	...
	if mode == "0":
		...
		return arr[arr[pos]]
	if mode == "2":
		# relative mode: use the relative_base
		return arr[arr[pos]] + relative_base
...
def handleAction(arr, pos=0):
	...
	if opcode == "09":
		...
		relative_base += getParam(arr, pos+1, command[2])
	#print(arr)
	return arr, nextpos
...
def main():
	with open("input.txt", "r", encoding="utf-8") as inp:
		...
		arr = []
		for item in rawdata:
			arr.append(long(item))
		print(arr)
		simulateMachine(arr)

#main()

simulateMachine([109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99])
